...
from simulators.placement import random_initial_state, OBJ_H, OBJ_SIZE, BIN_W, BIN_D

MARGIN = OBJ_SIZE * 0.7
LEVEL_Z = [OBJ_H + i * OBJ_SIZE for i in range(4)]  # [0.04, 0.12, 0.20, 0.28]
...
class TestTargetZLevelExplicit:
    ...
    def test_level0_always_floor(self):
        ...
        for seed in range(20):
            state = random_initial_state(n_obstacles=2, n_z_levels=2,
    ...
    def test_level1_height_and_xy_match_obstacle(self):
        ...
        for seed in range(20):
            state = random_initial_state(n_obstacles=1, n_z_levels=2,
    ...
    def test_level1_no_obstacles_falls_back_to_floor(self):
        ...
        for seed in range(10):
            state = random_initial_state(n_obstacles=0, n_z_levels=2,
                                         target_z_level=1, seed=seed)
            z = state['target_pos'][2].item()
            assert z == pytest.approx(OBJ_H, abs=1e-4), f"seed={seed}: got z={z}"
...
class TestTargetZLevelNone:
    ...
    def test_single_level_always_floor(self):
        """With n_z_levels=1, randomised target must land at floor."""
        for seed in range(20):
            state = random_initial_state(n_obstacles=2, n_z_levels=1,
                                         target_z_level=None, seed=seed)
            z = state['target_pos'][2].item()
            assert z == pytest.approx(OBJ_H, abs=1e-4), f"seed={seed}: got z={z}"

    def test_two_levels_valid_z(self):
        """With n_z_levels=2, target z must be one of the two valid level heights."""
        valid = LEVEL_Z[:2]
        for seed in range(20):
            state = random_initial_state(n_obstacles=1, n_z_levels=2,
                                         target_z_level=None, seed=seed)
...
class TestPlacementInvariants:
    ...
    def test_target_within_bin_bounds(self):
        ...
        for seed in range(50):
            state = random_initial_state(n_obstacles=3, n_z_levels=2,
                                         target_z_level=None, seed=seed)
            tx, ty = state['target_pos'][0].item(), state['target_pos'][1].item()
